...
class Utils():
    ...
    def dynamic_cutter(self, index_0, index_1):
        interval = 5
    ...
    def back_relation(self, frames, cut_indexes, iteration, parts_count, debug=False):
        if iteration != parts_count - 1:
            ...
            if debug:
                print(f'Размер кадра для переноса: {self.last_cadr.shape}')
                print(f'Перенесённая сегментация:  {self.last_yolo_pred}')
    ...
    def batch_correcting(self, batch_frames, predict, iteration, parts_count):
        ...
        print(f'Полный список индексов: {cut_indexes}')
        ...
        if iteration != parts_count - 1:
            ...
            if self.add_last_cadr == []:
                ...
                print(f'Восстановлено: {self.add_last_cadr.shape[0]} кадров')
            ...
            if self.add_last_cadr != []:
                ...
                print(f'Восстановлено на последней итерации: {self.add_last_cadr.shape[0]} кадров')
                cut_indexes[1:] += np.array(self.add_last_cadr.shape[0])

                self.add_last_cadr = np.array(self.sub_last_cadr)
                print(f'Записано на последней итерации: {self.sub_last_cadr.shape[0]} кадров')
